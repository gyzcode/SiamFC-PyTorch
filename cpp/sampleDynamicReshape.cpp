...
bool SampleDynamicReshape::buildPreprocessorEngine(const SampleUniquePtr<nvinfer1::IBuilder>& builder)
{
    ...
    profileCalib->setDimensions(input->getName(), OptProfileSelector::kMIN, Dims4{calibBatchSize, 3, 255, 255});
    profileCalib->setDimensions(input->getName(), OptProfileSelector::kOPT, Dims4{calibBatchSize, 3, 255, 255});
    profileCalib->setDimensions(input->getName(), OptProfileSelector::kMAX, Dims4{calibBatchSize, 3, 255, 255});
    ...
    if (mParams.int8)
    {
        ...
        MNISTBatchStream calibrationStream(calibBatchSize, nCalibBatches, "train-images-idx3-ubyte",
            "/home/gyz/dataset/otb100/*/img/0001.jpg", mParams.dataDirs);
...
bool SampleDynamicReshape::buildPredictionEngine(const SampleUniquePtr<nvinfer1::IBuilder>& builder)
{
    ...
    const int calibBatchSize{3};
    // We do not need to check the return of setDimension and setCalibrationProfile here as all dims are explicitly set
    profileCalib->setDimensions(inputName, OptProfileSelector::kMIN, Dims4{calibBatchSize, 3, 255, 255});
    profileCalib->setDimensions(inputName, OptProfileSelector::kOPT, Dims4{calibBatchSize, 3, 255, 255});
    profileCalib->setDimensions(inputName, OptProfileSelector::kMAX, Dims4{calibBatchSize, 3, 255, 255});
    ...
    saveEngine(*mPredictionEngine.get(), "./test.engine");
    return true;
}
...
samplesCommon::OnnxSampleParams initializeSampleParams(const samplesCommon::Args& args)
{
    ...
    if (args.dataDirs.empty()) //!< Use default directories if user hasn't provided directory paths
    {
        ...
        params.dataDirs.push_back("/home/gyz/workzone/siamfc-pytorch/pretrained/");
    }
